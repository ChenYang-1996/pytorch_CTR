...
class CriteoDataset(torch.utils.data.Dataset):
    ...
    #def __init__(self, dataset_path='.criteo/train.txt', cache_path='.criteo', rebuild_cache=False, min_threshold=10):
    def __init__(self, dataset_path='.criteo/train.txt', cache_path='../torchfm/dataset/criteo/.croteo', rebuild_cache=False, min_threshold=10):
        self.NUM_FEATS = 39
        ...
        self.env = lmdb.open(cache_path, create=False, lock=False, readonly=True) #False,False,True
        with self.env.begin(write=True) as txn: #False
            self.length = txn.stat()['entries'] - 1
            self.field_dims = np.frombuffer(txn.get(b'field_dims'), dtype=np.uint32)

    def __getitem__(self, index):
        with self.env.begin(write=False) as txn:#False
