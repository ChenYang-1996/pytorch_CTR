...
class DeepCrossAttentionalProductNetwork(torch.nn.Module):
    def __init__(self, field_dims, embed_dim, num_heads, num_layers, mlp_dims, dropouts):
        ...
        # self.cap = CrossAttentionalProductNetwork(num_fields, embed_dim, num_heads, num_layers=num_layers,
        #                                           ffn_embed_dim=400, dropout=dropouts[0], activation_fn='relu', normalize_before=True) #ffn_embed_dim
        self.cap = CrossAttentionalProductNetwork(num_fields, embed_dim, num_heads, num_layers=num_layers, dropout=dropouts[0])
    ...
    def forward(self, x):
        ...
        """
        device = x.device
        #attn_mask = self.generate_square_subsequent_mask(torch.tensor(x.size(1))*self.num_heads).to(device) #reedited by yc
